...
export default class Ball {
    ...
    #pong;
    #speed = 5;
    #positionLine = new Line(new Vector(0, 0), new Vector(0, 0));
    #size;
    ...
    constructor({ size, canvas, pong, }) {
        ...
        for (let i = angle; i <= Math.PI * 2; i += angle)
            this.#diagonals.push(new Line(this.#position, Vector.fromPolar(this.#size, +i.toFixed(4)).add(this.#position)));
    }
    reset() {
    ...
    #edges() {
        for (const diagonal of [this.#positionLine, ...this.#diagonals]) {
            ...
            for (const boundary of this.#pong.boundaries) {
                ...
                if (intersection) {
                    // do resolution first
                    this.#position.subtract(Vector.subtract(diagonal.end, diagonal.start).multiply(1 - intersection.offset1));
            ...
            for (const paddle of this.#pong.paddles) {
                ...
                if (paddleIntersection) {
                    // do resolution first
                    this.#position.subtract(Vector.subtract(diagonal.end, diagonal.start).multiply(1 - paddleIntersection.offset1));
                    // reflect the velocity vector by this boundary
    ...
    #drawVectors() {
        // velocity
        this.canvas
            .line(this.#position, Vector.multiply(this.#direction, Math.max(innerWidth, innerHeight)).add(this.#position))
            .stroke({ color: "#0f0" });
        // position line
        // this.canvas
